...
(OptFS~\citep{Chidambaram2013OptFS}); ordering approaches --- soft
...
time~\citep{LeBlanc2024SquirrelFS} --- avoid a journal but have no single atomic
...
inserted byte does not shift all following boundaries --- at the cost of
...
directory structure; Ivy needs a conflict tool). At the other pole, CRDTs
...
(strain splits) --- concurrent edits to different fragments of the same file
...
(HbC) systems --- CFS~\citep{Blaze1993CFS}, \texttt{gocryptfs}/eCryptfs,
dm-crypt/LUKS --- give at-rest confidentiality for one user, without multi-writer
...
forks versions --- an explicit model weakening we defend in \cref{sec:discussion}.
